...
from app.shib import get_status
...
def send_email(email, token, template_id, recipient, seed=None):
    ...
    dict = {"coupon": token, "expireDate": date_time, "user": recipient}
    logger.debug(f"coupon: {token}, recipient: {recipient}")
    if seed:
        dict["seed"] = seed
...
def generate_sms_message(coupon):
    message = (
        "Please click to join Snowball Study: https://snowballgr.duhs.duke.edu/redeem?coupon="
        + coupon
...
def oauth_authentication():
    # logger.info("Duke Shibboleth oauth login")
    authorization_url, state = get_status()
