...
class UploadCSV(Resource):
    def post(self):
        ...
        for index, row in df.iterrows():
            try:
                db_utils.insert_seed(
                    _create_seed_report_row(row),
                    app.config["TEST_RESULT_DATE_FORMAT_CSV"],
                )
                count += 1
                ...
                rejected += 1
                error_list.update({"row": index + 1, "errorMsg": error_msg})
            except Exception as e:
                logger.error(f"Failed to save row {index+1}: {str(e)}")
                rejected += 1
                error_list.update({"row": index + 1, "errorMsg": str(e)})
...
def _create_seed_report_row(csv_row):
    ...
    return {
        **{
            ...
            # if key in app.config["CSV_HEADERS"]
        },
...
class AddNewSeed(Resource):
    ...
    def post(self):
        ...
        try:
            db_utils.insert_seed(
                request_data, app.config["TEST_RESULT_DATE_FORMAT_ADD"]
            )
            return utils.response_with_status_code(
...
def _sanity_check(request_data, add=False):
    ...
    if "MRN" not in request_data:
        ...
        if add:
            ...
            logger.debug("Writing to db: " + dumps(request_data))
        return None
