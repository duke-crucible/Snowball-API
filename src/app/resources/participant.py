...
class CohortReport(Resource):
    def get(self):
        ...
        return _process_get_participants(
            q, app.config["COHORT_REPORT_FIELDS"], cohor=True
        )
...
class Survey(Resource):
    ...
    def post(self):
        data = request.get_json()
        submit = data.get("completed", "no") if data else False
        return _process_post_requests(
            data,
            participant_only=False,
            coll="survey",
            time_field=app.config["SURVEY_COMPLETION_DATE"]
            if submit
            else None,
        )
...
class Participants(Resource):
    def get(self):
        ...
        q = {
            ...
            # app.config["ENROLLMENT_COMPLETED"]: {"$exists": True},
...
def _process_post_requests(data, participant_only=True, coll=None, time_field=None):
    ...
    try:
        ...
        logger.debug(f"process requests: {data}")
        if participant_only:
            ...
            if coll == "survey" or coll == "consent":
                ...
                # TODO: need to decide which usecase requires
                # using record_id as key to avoid duplidates
                data["_id"] = utils.record_id_str(record_id)
...
def _process_get_participants(query, fields_to_get, cohor=False, contacts=None):
    error_msg = None
    try:
    ...
    if error_msg is None:
        if cohor:
            for participant in participants:
                ...
                # if fn != "":
                #     del participant["FIRST_NAME"]
                # if ln != "":
                #     del participant["LAST_NAME"]
        elif contacts == "y":
            data = []
            for participant in participants:
