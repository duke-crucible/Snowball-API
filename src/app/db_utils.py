...
def import_data_from_json(file, collection_name, db_name=None):
    db = get_db_handle(db_name)
...
def _append_data():
    db = get_db_handle()
    coll_name = app.config["COLLECTIONS"].get("seeds")
    coll = db[coll_name]

    query = {"MRN": "MRN4635894"}
    coll.update_one(query, {"$push": {"REPORT_DATE": "2021-03-17"}})
...
def _get_one(collection_name, query, fields):
    db = get_db_handle()
    coll = db[collection_name]
    logger.debug(f"query: {query}, fields: {fields}")
...
def _update_one(collection_name, query, updates, insertIfNotExists=False):
    db = get_db_handle()
    coll = db[collection_name]
    logger.debug(f"query: {query}, updates: {updates}, upsert: {insertIfNotExists}")
...
def insert_seed(doc, format):
    # Set _id to MRN to ensure MRN is unique
    doc["_id"] = doc["MRN"]
    field = app.config["RESULT_DATE"]
    if field in doc:
        # Use datetime format for result date field
        doc[field] = datetime.strptime(doc[field], format)
...
def get_record(record_id, coll):
    query = {"_id": utils.record_id_str(record_id)}
    _get_one(app.config["COLLECTIONS"].get(coll), query, {"_id": 0})
...
def get_latest(collection, sort_field, field=None):
    ...
    logger.debug(f"extract_data_field: {extract_data_field}")
    temp = list(
...
def get_current_consent_version():
    db = get_db_handle()
    coll = db["Consent.files"]
    return len(list(coll.find({"filename": "consent-form.pdf"})))


def get_latest_consent_form():
    fs = GridFS(get_db_handle(), app.config["COLLECTIONS"].get("consentform"))
    return fs.get_last_version(filename="consent-form.pdf")
